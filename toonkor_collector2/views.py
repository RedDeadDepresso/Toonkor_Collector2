...
class BrowseManhwaView(View):
    def get(self, request, manhwa_slug):
        if Manhwa.objects.filter(slug=manhwa_slug):
            return redirect(
                reverse(
                    "toonkor_collector2:library_manhwa",
                    kwargs={"manhwa_slug": manhwa_slug},
                )
            )
        else:
...
class AddLibrary(View):
    def get(self, request):
        ...
        redirect_url = reverse(
            "toonkor_collector2:library_manhwa", kwargs={"manhwa_slug": manhwa_slug}
        )
...
class RemoveLibrary(View):
    def get(self, request):
        ...
        redirect_url = reverse(
            "toonkor_collector2:browse_manhwa", kwargs={"manhwa_slug": manhwa_slug}
        )
        return JsonResponse({"status": "success", "redirect": redirect_url})
